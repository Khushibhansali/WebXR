...
var allPositionShifted = false;

var targetPositions = [0, 1, 2, 3, 4, 5, 6, 7, 8];
...
//check if this truly works
function isConverged(obj) {
    Object.keys(obj).forEach((key) => {
        //console.log("converged", key, obj[key])
        if (obj[key] < 7){
            allPositionShifted = false;
        }else{
            allPositionShifted = true;
        }
    })};
...
async function newTrial(response) {
   ...
   // document.getElementById("bottom-text").setAttribute("text", "value", "\n\n" + (responses.length) + "/" + trials);
    ...
    setTimeout(async function () {
        // console.log("all shifted", allPositionShifted)
        isConverged(positionShifts);
        if(allPositionShifted) {
            // END EXPERIMENT!
    ...
    if(allPositionShifted){
        ...
        Object.keys(positionContrastValues).forEach((key)=>{
            responses.push({
                targetName: key,
                contrast: positionContrastValues[key],
